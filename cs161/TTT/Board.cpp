...
bool Board::makeMove(int xCoord, int yCoord, char thisPlayer){
    ...
    std::cout << "PreMovecheck = " << preMoveCheck << std::endl;
    ...
    switch (preMoveCheck){
      ...
      default:          break;
        ...
        std::cout << "("<<xCoord<<", "<<yCoord<<", "<<thisPlayer<< ")" << std::endl;
           
         return true;
       
    }

//runs after T3Reader::readGameFile() finishes
//if T3Reader::readGameFile() returns true, it means that enum Gamestate = 0, 1, or 2.
//if readgameFile() returns false, it means that enum Gamestate = 3
...
GameResult Board::gameState(){
    int pointTally = 0;
    for (int diagChecker = 0; diagChecker < 3; diagChecker++){
        pointTally += gameBoard[0][diagChecker];
        if (pointTally == 360)
            return X_WON;
        if (pointTally == 333)
            return O_WON;
    ...
    pointTally = 0;
    for (int diagChecker = 2; diagChecker >-1 ; diagChecker--){
        pointTally += gameBoard[2][diagChecker];
        if (pointTally == 360)
            return X_WON;
        if (pointTally == 333)
            return O_WON;
    ...
    for (int counterCol = 0; counterCol < 3; counterCol++){
        for (int counterRow = 0; counterRow < 3; counterRow++){
          std::cout << "running gameState, Col = "<< counterCol<< "row = " << counterRow<< "current array val: " << gameBoard[counterCol][counterRow] << std::endl;
